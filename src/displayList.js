...
class DisplayList extends React.Component {
  render() {
    ...
    var renderedSortedItems = sortedItems.map((item, index) => {
      return (
        <DisplayListItem
          index={index}
          ...
          handleEditEndDate={this.props.handleEditEndDate}
          selectedButton={this.props.selectedButton}
          prioritiseThis={swapArray(this.props.items, index, index - 1)}
          editThis={editThis}
          key={index}
          handleEditStartDate={this.props.handleEditStartDate}
        />
      );
    });
